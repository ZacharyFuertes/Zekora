...
export function HiddenVaultGate({ onReady }: { onReady?: () => void }) {
  ...
  if (unlocked) return <div className="relative mx-auto min-h-80 max-w-4xl overflow-hidden rounded-3xl border-2 border-dashed border-neon/30 bg-surface p-8 text-center shadow-[0_0_32px_rgba(168,85,247,0.08)]"><div className="mx-auto max-w-md"><LockKeyhole className="mx-auto h-12 w-12 text-neon" /><p className="mt-5 font-pixel text-sm uppercase text-text">Hidden vault unlocked</p><p className="mt-2 text-sm text-text-muted">Your private vault session is active.</p><button type="button" onClick={() => void logout()} className="mt-6 inline-flex items-center gap-2 rounded-lg border-2 border-border px-4 py-2 font-pixel text-[10px] uppercase text-text-muted hover:border-danger hover:text-danger"><LogOut className="h-4 w-4" /> Lock vault</button></div></div>
    ...
    return <div className="mx-auto max-w-md rounded-3xl border-2 border-accent/30 bg-surface p-6 shadow-[0_0_32px_rgba(221,44,0,0.08)] sm:p-8"><div className="text-center"><KeyRound className="mx-auto h-10 w-10 text-accent" /><p className="mt-4 font-pixel text-sm uppercase text-accent">{mode === "signup" ? "Create your Hidden Vault" : "Unlock your Hidden Vault"}</p><p className="mt-2 text-sm text-text-muted">{mode === "signup" ? "Create a separate account for your most private files." : "Sign in to access your private vault."}</p></div><form onSubmit={submit} className="mt-6 space-y-4"><input type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="EMAIL ADDRESS" required disabled={mode === "login"} className="w-full rounded-lg border-2 border-border bg-bg px-3 py-3 font-pixel text-[10px] text-text outline-none focus:border-accent disabled:opacity-60" /><div className="relative"><input type={showPassword ? "text" : "password"} value={password} onChange={(event) => setPassword(event.target.value)} placeholder="HIDDEN VAULT PASSWORD" required minLength={8} className="w-full rounded-lg border-2 border-border bg-bg px-3 py-3 pr-11 font-pixel text-[10px] text-text outline-none focus:border-accent" /><button type="button" onClick={() => setShowPassword((value) => !value)} aria-label={showPassword ? "Hide password" : "Show password"} className="absolute right-2 top-1/2 -translate-y-1/2 p-2 text-text-muted hover:text-accent">{showPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</button></div>{message && <p className="font-pixel text-[9px] uppercase text-danger">{message}</p>}<button type="submit" disabled={submitting} className="flex w-full items-center justify-center gap-2 rounded-lg border-2 border-accent bg-accent py-3 font-pixel text-[10px] font-bold uppercase text-bg hover:bg-[#c62800] disabled:opacity-50">{submitting && <Loader2 className="h-4 w-4 animate-spin" />}{mode === "signup" ? "Create vault account" : "Unlock vault"}</button></form><button type="button" onClick={() => setMode(mode === "signup" ? "login" : "signup")} className="mt-5 w-full font-pixel text-[10px] uppercase text-accent hover:underline">{mode === "signup" ? "Already have a Hidden Vault? Sign in" : "Need a Hidden Vault? Create one"}</button></div>
}
